...
def main():
    ...
    output_lock = data_settings['data']['dynamic']['outcome']['lock_folder']
    ...
    for domain in data_settings["data"]["dynamic"]["fp"]["domains"]:
        ...
        paths["output"] = output_lock.format(**dict_filled)
        ...
        out_flood_ancillary_map = os.path.join(paths["ancillary"], data_settings["data"]["dynamic"]["ancillary"]["file_name_max"]).format(**dict_filled)

        if not os.path.isfile(out_flood_ancillary_map):
            # If maximum value has not been computed yet, read and calculate max discharge
            ...
            logging.info(" ----> Maximum map has already been computed... Open it!")
            dis_max = check_raster(xr.open_rasterio(out_flood_ancillary_map))
        ...
        mask = np.where(area_km >= data_settings["data"]["dynamic"]["thresholds"]["area_km2"], 1, 0)

        th_map_file = xr.open_rasterio(data_settings["data"]["static"]["discharge_threshold"].format(domain=domain))
        th_map = check_raster(th_map_file)
        th_map[th_map <= 0] = np.Inf
        alert_map = np.where(((dis_max > th_map) & (dis_max >= data_settings["data"]["dynamic"]["thresholds"]["discharge_min"])), 1, 0)
        alert_map = np.where(mask == 1, alert_map, 0)

        # Check results
        if np.nansum(alert_map) > 0:
            logging.info(" ---> Some part of the river network exceeds the selected treshold!")
            with open(os.path.join(paths["output"], "hmc_probabilistic_{outcome_datetime}_ACTIVE.txt").format(**dict_filled), 'w') as f:
                f.write("---> " + str(np.nansum(alert_map)) + " pixels of the network are over-treshold")
        else:
            logging.info(" ---> No overcome of the probabilistic trigger treshold in the domain!")
            with open(os.path.join(paths["output"], "hmc_probabilistic_{outcome_datetime}_NOT_ACTIVE.txt").format(**dict_filled), 'w') as f:
                f.write("---> " + str(np.nansum(alert_map)) + " pixels of the network are over-treshold")
